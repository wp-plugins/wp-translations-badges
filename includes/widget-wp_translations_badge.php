...
class Widget_wp_translations_badge extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			__( 'WP-Translations Badges', 'wp_translations_badges' ),
			array(
				'description'	=>	__( 'Display a WP-Translations Badge.', 'wp_translations_badges' )
			)
		);
	...
	public function widget( $args, $instance ) {
		...
		$element = Wp_Translations_Badges::get_instance();
		echo $element->render_element($instance, '', 'wp_translations_badge');
	...
	public function form( $instance ) {
		...
		//$configfiles = glob( self::get_path( dirname( __FILE__ ) ) .'configs/wp_translations_badge-*.php' );
		if(file_exists(self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-wp_translations_badge.php')){
			include self::get_path( dirname( __FILE__ ) ) .'configs/fieldgroups-wp_translations_badge.php';
		}else{
			return;
		}

		echo "<input type=\"hidden\" name=\"wp_translations_badges-widget\">\r\n";
		$groups = array();
		$setsize = 'full';
		...
		foreach ($configfiles as $key=>$fieldfile) {
			include $fieldfile;
			$group['id'] = uniqid('wp_translations_badges');
			$groups[] = $group;
		}


			echo "<div class=\"wp_translations_badges-widget-config-content " . $setsize . "\">\r\n";
			foreach ($groups as $key=>$group) {
				echo "<div id=\"row".self::get_field_id('__row'.$group['id'])."\" class=\"wp_translations_badges-groupbox group\" " . ( !empty($instance['__cur_tab__']) ? ($instance['__cur_tab__'] == $key ? "" : "style=\"display:none;\"") : ($key === 0 ? "" : "style=\"display:none;\"" )) . ">\r\n";
	...
	public function group($group, $instance){
		...
		for( $i=0; $i<$depth;$i++ ){
				if($i > 0){
					echo '  <div class="button button-primary right wp_translations_badges-removeRow" style="margin:5px 5px 0;">'.__('Remove', 'wp_translations_badges').'</div>';
				}
		...
		if(!empty($group['multiple'])){
			echo "<div><button class=\"button wp_translations_badges-add-group-row\" type=\"button\" data-field=\"".self::get_field_id('ref')."\" data-rowtemplate=\"group-".$group['id']."-tmpl\">".__('Add Another','wp_translations_badges')."</button></div>\r\n";
		}
		...
		foreach ($configfiles as $key=>$fieldfile) {
			...
			if(!empty($group['scripts'])){
				foreach($group['scripts'] as $script){
					wp_enqueue_script( 'wp_translations_badges-'.strtok($script, '.'), self::get_url( 'assets/js/'.$script , dirname(__FILE__) ) , array('jquery') );
				}
			}
			if(!empty($group['styles'])){
				foreach($group['styles'] as $style){
					wp_enqueue_style( 'wp_translations_badges-'.strtok($style, '.'), self::get_url( 'assets/css/'.$style , dirname(__FILE__) ) );
				}
			}
		}

		wp_enqueue_style( 'wp_translations_badges-panel-styles', self::get_url( 'assets/css/panel.css', dirname(__FILE__) ) );
		wp_enqueue_script( 'wp_translations_badges-panel-script', self::get_url( 'assets/js/panel.js', dirname(__FILE__) ), array( 'jquery' ) );
				...
				if(is_active_sidebar($sidebarid) && $sidebarid != 'wp_inactive_widgets' && false === strpos($sidebarid, 'orphaned_widgets')){
					foreach($widget_settings as $key=>$settings){
						if(in_array($this->id_base.'-'.$key,$sidebar)){
							...
							echo $element->render_element($settings, '', 'wp_translations_badge', true);
...
add_action( 'widgets_init', create_function( '', 'register_widget("Widget_wp_translations_badge");' ) );
